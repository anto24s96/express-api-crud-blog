...
const store = (req, res) => {
    if (
        ...
        if (!req.file || !req.file.mimetype.includes("image")) {
            ...
            return res
                .status(400)
                .send("Image is missing or it is not an image file");
        }
...
const destroy = (req, res) => {
    ...
    if (!postToDelete) {
        ...
        return res.format({
            ...
            json: () =>
                res
                    .status(404)
                    .json({ error: `Post not found with slug: ${slug}` }),
        });
    }
    ...
    res.format({
        html: () =>
            res.send(`The post with slug: ${slug} was successfully deleted`),
        json: () =>
            res.json({
                message: `Post with slug ${slug} successfully deleted`,
            }),
    });
};
...
const downloadImage = (req, res) => {
    ...
    if (!post || !post.image) {
        return res.status(404).json({
            ...
            description: post
                ? "Image not found"
                : `Post with slug: ${postSlug} does not exist`,
        });
    }
